// dropout_bwd.wgsl — Inverted dropout backward (same mask as fwd).
// dx[i] = gout[i] * (hash(i, seed) >= p ? 1/(1-p) : 0)
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  if (params.p <= 0.0) { dx[i] = gout[i]; return; }
  let h = hash2(i, params.seed);
  let r = f32(h >> 8u) / 16777215.0;
  if (r < params.p) { dx[i] = 0.0; }
  else { dx[i] = gout[i] / (1.0 - params.p); }
}
